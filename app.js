...
/*
 const userSearch = document.getElementById('userSearch');
    if (userSearch) {
        console.log('Setting up userSearch event listener');
        
        // Remove any existing event listeners by cloning
        const newSearchInput = userSearch.cloneNode(true);
        userSearch.parentNode.replaceChild(newSearchInput, userSearch);
        
        // Add keypress listener for Enter key
        newSearchInput.addEventListener('keypress', (e) => {
            if (e.key === 'Enter') {
                e.preventDefault();
                console.log('Enter pressed in search, calling app.searchUsers()');
                app.searchUsers();
            }
        });
        
        // Add input listener for real-time search (optional)
        newSearchInput.addEventListener('input', (e) => {
            // You can add debounced search here if desired
...
if (document.getElementById('userSearch')) {
        ...
        const searchBtn = document.querySelector('button[onclick*="searchUsers"]');
        if (searchBtn) {
            searchBtn.onclick = null; // Clear any existing
...
printPurchaseReport() {
    ...
    const { todayTransactions, currentUser, today, isAdmin } = this.currentPurchaseData;
    
    let printContent = `
        <html>
        <head>
            <title>Purchase Report - ${today}</title>
            <style>
                ...
                .print-table th { background: #f0f0f0; padding: 10px; text-align: left; border: 1px solid #ddd; }
                .print-table td { padding: 8px; border: 1px solid #ddd; }
                .print-summary { margin-top: 30px; padding: 15px; background: #f8f9fa; border: 1px solid #ddd; }
                .print-footer { margin-top: 30px; padding-top: 10px; border-top: 1px solid #ddd; text-align: center; font-size: 12px; color: #666; }
        ...
        <body>
            <div class="print-header">
                ...
                <p>Date: ${today} | Generated by: ${currentUser.userID}</p>
                ${isAdmin ? '<p><strong>👑 Admin View - All Users</strong></p>' : '<p><strong>👤 User View - Your Purchases Only</strong></p>'}
            </div>
            
            <div class="print-info">
                <div>
                    <strong>Total Transactions:</strong> ${todayTransactions.length}<br>
    ...
    if (todayTransactions.length > 0) {
        // Calculate totals
        const totalAmount = todayTransactions.reduce((sum, t) => sum + (parseFloat(t.amount) || 0), 0);
        const totalQuantity = todayTransactions.reduce((sum, t) => sum + (parseInt(t.quantity) || 0), 0);
        ...
        todayTransactions.forEach((transaction, index) => {
            const date = new Date(transaction.serverTimestamp || transaction.timestamp || transaction.purchaseDate);
            const userID = transaction.performedBy || transaction.userID || 'Unknown';
            ...
            printContent += `
                <tr>
                    ...
                    ${isAdmin ? `<td>${userID}</td>` : ''}
                    <td>${transaction.productName || 'Unknown'}</td>
                    <td>${transaction.barcode || 'N/A'}</td>
                    <td>${transaction.quantity || 0}</td>
                    <td>₦${(transaction.unitPrice || 0).toFixed(2)}</td>
                    <td class="amount">₦${(transaction.amount || 0).toFixed(2)}</td>
                    <td>${transaction.supplier || 'Unknown'}</td>
                </tr>
            `;
        });
        ...
        printContent += `
            ...
            <div class="print-summary">
                <h3>Summary</h3>
                <p><strong>Total Transactions:</strong> ${todayTransactions.length}</p>
                <p><strong>Total Items Purchased:</strong> ${totalQuantity}</p>
                <p><strong>Total Purchase Amount:</strong> ₦${totalAmount.toFixed(2)}</p>
                ${isAdmin ? `<p><strong>Unique Users:</strong> ${new Set(todayTransactions.map(t => t.performedBy || t.userID)).size}</p>` : ''}
            </div>
        `;
    } else {
        printContent += `
            <div style="text-align: center; padding: 50px; color: #666;">
                <h2>No purchases recorded for today</h2>
                <p>Date: ${today}</p>
            </div>
        `;
    }
...
// In app.js, replace the getPurchaseReport() method with this:
async getPurchaseReport() {
    try {
        ...
        if (isAdmin) {
            ...
            todayTransactions = allTransactions.filter(transaction => {
                if (!transaction.performedBy && !transaction.userID) {
                    console.warn('Purchase transaction missing userID:', transaction.id);
                    return false;
                }
        ...
        // Create pagination instance
        this.purchaseReportPagination = new ReportPagination(todayTransactions, 10);
        ...
        return `
            <div class="content-page">
                <div class="error-message">
                    <h3>Error Loading Purchase Report</h3>
                    <p>Unable to load purchase data. Please try again.</p>
                    <pre class="error-details">${error.message}</pre>
...
// Add renderPurchaseReport() method:
renderPurchaseReport() {
    if (!this.purchaseReportPagination || !this.currentPurchaseData) {
        return '<div class="error-message">Report data not loaded</div>';
    }
    ...
    const pageTransactions = this.purchaseReportPagination.getCurrentPageItems();
    const pageInfo = this.purchaseReportPagination.getPageInfo();
    
    // Calculate totals from ALL transactions (not just current page)
    const totalPurchases = todayTransactions.reduce((sum, transaction) => {
        return sum + (parseFloat(transaction.amount) || 0);
    }, 0);
    
    const totalItems = todayTransactions.reduce((sum, transaction) => {
        return sum + (parseInt(transaction.quantity) || 0);
    }, 0);
    ...
    const adminHeader = isAdmin ? `
        <div class="admin-view-indicator">
            <strong>👑 Admin View - All Purchases Today</strong>
            <p>You are viewing ALL purchase transactions for today from all users.</p>
            <div class="admin-stats">
                <span>📊 Total Transactions: ${todayTransactions.length}</span>
                <span>👥 Total Users: ${Object.keys(transactionsByUser).length}</span>
                <span>📄 Current Page: ${pageInfo.currentPage}/${pageInfo.totalPages}</span>
    ...
    const userHeader = !isAdmin ? `
        <div class="user-view-indicator">
            <strong>👤 Your Purchases Today</strong>
            <p>You are viewing only your purchase transactions for today.</p>
            <div class="user-info">
                <span>User ID: ${currentUserID}</span>
                <span>Total Transactions: ${todayTransactions.length}</span>
                <span>Current Page: ${pageInfo.currentPage}/${pageInfo.totalPages}</span>
        ...
        return `
            <div class="content-page report-container">
                ...
                <div class="no-purchases-message">
                    ...
                    ${isAdmin ?
                        '<p><em>As an admin, you would see all users\' purchases here.</em></p>' :
                        '<p><em>Only your purchases are shown here.</em></p>'
                    }
    ...
    // Sort page transactions by timestamp
    const sortedPageTransactions = [...pageTransactions].sort((a, b) => {
        const timeA = new Date(a.serverTimestamp || a.timestamp || a.purchaseDate);
        const timeB = new Date(b.serverTimestamp || b.timestamp || b.purchaseDate);
        return timeB - timeA;
    });
        ...
        <div class="content-page report-container">
            ...
            <div class="report-header">
                <div>
                    ...
                    <p class="report-date">
                        Date: ${today} |
                        ${isAdmin ? '👑 Admin View (All Users)' : '👤 User View (Your Purchases Only)'}
                    </p>
                </div>
                <div class="export-actions">
            ...
            ${isAdmin && Object.keys(transactionsByUser).length > 0 ? `
                <div class="admin-summary-section">
                    <h3><span>📊</span> Purchases Breakdown by User</h3>
                    <div class="user-breakdown-grid">
                        ${Object.entries(transactionsByUser).map(([userID, data]) => `
                            <div class="user-breakdown-card">
                                <div class="user-header">
            ...
            <!-- Pagination Top -->
            ${todayTransactions.length > 10 ? this.purchaseReportPagination.getPaginationHTML('app.paginatePurchaseReport') : ''}

            <!-- Transactions List -->
            <h3>Today's Purchase Transactions${isAdmin ? ' (All Users)' : ' (Your Purchases)'}</h3>
            <p class="page-info-small">Showing ${pageInfo.startItem}-${pageInfo.endItem} of ${pageInfo.totalItems} transactions</p>
            ...
            <div class="purchases-list">
                ${sortedPageTransactions.map((transaction, index) => {
                    ...
                    return `
                    <div class="purchase-item ${isCurrentUser ? 'current-user-transaction' : 'other-user-transaction'}">
                        <div class="purchase-header">
                            <span class="purchase-number">${absoluteIndex + 1}.</span>
                            <span class="purchase-time">
                                ${new Date(transaction.serverTimestamp || transaction.timestamp || transaction.purchaseDate).toLocaleTimeString([], {
                                    hour: '2-digit',
                                    minute: '2-digit'
                                })}
                            </span>
                            ${userBadge}
                            <span class="purchase-id">ID: ${transaction.transactionId || transaction.id || 'N/A'}</span>
                        </div>
                        ${businessInfo}
                        <div class="purchase-details">
                            <div class="purchase-product">
                                ...
                                <div class="purchase-description">
                                    <span>Barcode: ${transaction.barcode || 'N/A'}</span>
                                    <span>Quantity: ${transaction.quantity || 0}</span>
                                    <span>Unit Price: ₦${(transaction.unitPrice || 0).toFixed(2)}</span>
                                </div>
                            </div>
                            <div class="purchase-amount">
                                <span class="amount-label">Total:</span>
                                <span class="amount-value">₦${(transaction.amount || 0).toFixed(2)}</span>
                            </div>
                        </div>
                        <div class="purchase-meta">
                            ${transaction.supplier ? `<span class="meta-item">Supplier: ${transaction.supplier}</span>` : ''}
                            ${transaction.type ? `<span class="meta-item">Type: ${transaction.type}</span>` : ''}
                            ${transaction.notes ? `<span class="meta-item">Note: ${transaction.notes}</span>` : ''}
                        </div>
                    </div>
                    `;
                }).join('')}
            ...
            <div class="report-footer">
                ...
                <div class="export-buttons">
                    <button class="btn-secondary" onclick="app.exportPurchasesToCSV()">
                        📥 Download as CSV
                    </button>
                    <button class="btn-secondary" onclick="app.exportPurchasesToExcel()">
                        📊 Download as Excel
                    </button>
                    <button class="btn-secondary" onclick="app.printPurchaseReport()">
                        🖨️ Print Report
                    </button>
                </div>
            </div>
...
// In app.js, replace the getWalletAdminForm() method:
getWalletAdminForm() {
    return `
        <div class="wallet-admin-container">
            <div class="wallet-admin-header">
                <h2><span class="menu-icon">💰</span> Wallet Administration</h2>
                <p class="subtitle">Manage user wallet balances and adjustments</p>
            </div>
            ...
            <div class="search-section">
                <div class="search-box">
                    <input type="text" 
                           id="userSearch" 
                           placeholder="Search by User ID, Name, Email, or Business..."
                           autocomplete="off">
                    ...
                    <button class="clear-btn" onclick="app.clearSearch()">
                        <span class="menu-icon">🔄</span> Clear
                    </button>
                </div>
            ...
            <div class="users-section">
                <div class="section-header">
                    ...
                    <div class="section-actions">
                        <button class="btn-small" onclick="app.refreshUsersList()">
            ...
            <div class="selected-user-panel" id="selectedUserPanel" style="display: none;">
                ...
                <div class="user-details">
                    <div class="detail-item">
                        ...
                        <span class="detail-value" id="selectedUserID">-</span>
                    </div>
                    <div class="detail-item">
                        <span class="detail-label">Full Name:</span>
                        <span class="detail-value" id="selectedFullName">-</span>
                    </div>
                    <div class="detail-item">
                        <span class="detail-label">User Group:</span>
                        <span class="detail-value" id="selectedUserGroup">-</span>
                    </div>
                    <div class="detail-item">
                        <span class="detail-label">Current Balance:</span>
                        <span class="detail-value amount" id="selectedBalance">₦0.00</span>
                    </div>
                </div>
            </div>
            ...
            <div class="adjustment-form">
                <div class="form-header">
                    ...
                    <p>Select a user first to perform wallet adjustments</p>
                </div>
                
                <form id="walletAdjustmentForm">
                    <div class="form-grid">
                        <div class="form-group">
                            <label for="adjustmentAction">Action Type *</label>
                            <select id="adjustmentAction" name="action" required 
                                    onchange="app.handleActionChange()">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" 
                                   ...
                                   placeholder="0.00"
                                   disabled>
                            <div class="form-hint">Required for Add, Deduct, and Set</div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" 
                               ...
                               name="reason" 
                               required
                               placeholder="e.g., Salary payment, Expense deduction, etc.">
                    </div>
                    
                    <div class="form-group">
                        <label for="adjustmentNotes">Additional Notes</label>
                        <textarea id="adjustmentNotes" 
                                  name="notes" 
                                  rows="2"
                                  placeholder="Any additional details or comments..."></textarea>
                    </div>
                    
                    <div class="form-actions">
                        <button type="button" 
                                class="btn-primary" 
                                id="executeAdjustmentBtn"
                                onclick="app.executeWalletAdjustment()"
                                disabled>
                            <span class="menu-icon">✅</span> Execute Adjustment
                        </button>
                        <button type="button" 
                                class="btn-secondary"
                                onclick="app.resetAdjustmentForm()">
                            <span class="menu-icon">🗑️</span> Clear Form
                        </button>
                    </div>
                </form>
                ...
                <!-- Status Messages -->
                <div id="adjustmentStatus" class="status-message" style="display: none;"></div>
            </div>
            ...
            <div class="adjustments-history">
                <div class="section-header">
                    ...
                    <div class="section-actions">
                        <button class="btn-small" onclick="app.refreshAdjustments()">
                            <span class="menu-icon">🔄</span> Refresh
                        </button>
                        <button class="btn-small" onclick="app.exportAdjustments()">
...
// In app.js, update the searchUsers() method (the UI one) to ensure proper parameter passing:
// In app.js, update the searchUsers() method (UI version):
async searchUsers() {
    console.log('[UI] searchUsers called');
    ...
    try {
        ...
        console.log(`[UI] Search term from input: "${searchTerm}"`);
        
        const usersList = document.getElementById('usersList');
        if (!usersList) {
            console.error('[UI] usersList element not found');
            return;
        }
        ...
        // Call API search
        console.log(`[UI] Calling api.searchUsers("${searchTerm}")`);
        const users = await api.searchUsers(searchTerm);
        console.log(`[UI] API returned ${users.length} users`);
        ...
        console.log(`[UI] Rendered ${users.length} users`);
        
    } catch (error) {
        console.error('[UI] Error in searchUsers:', error);
...
// In app.js, add this test function:
// In app.js, replace testUserSearch() with this:
async testUserSearch() {
    console.log('=== TESTING USER SEARCH ===');
    
    try {
        // Test with empty search (should return all users)
        console.log('Test 1: Empty search (all users)');
        ...
        console.log('First user in database:', {
            ...
            businessName: firstUser.businessName
        });
        
        // Test 2: Search by exact userID
        console.log(`\nTest 2: Searching for exact userID "${firstUser.userID}"`);
        const exactSearch = await api.searchUsers(firstUser.userID);
        console.log(`Found ${exactSearch.length} users`);
        
        // Test 3: Search by partial userID
        console.log(`\nTest 3: Searching for partial "tmp"`);
        const partialSearch = await api.searchUsers('tmp');
        console.log(`Found ${partialSearch.length} users`);
        
        // Test 4: Search by email
        console.log(`\nTest 4: Searching for email "${firstUser.email}"`);
        const emailSearch = await api.searchUsers(firstUser.email);
        console.log(`Found ${emailSearch.length} users`);
        
        // Test 5: Search by full name
        console.log(`\nTest 5: Searching for name "Demo"`);
        const nameSearch = await api.searchUsers('Demo');
        console.log(`Found ${nameSearch.length} users`);
        
        // Test 6: Search for non-existent user
        console.log(`\nTest 6: Searching for "NONEXISTENT"`);
        const notFound = await api.searchUsers('NONEXISTENT');
        console.log(`Found ${notFound.length} users (should be 0)`);
